...
class SelfImitation(object):
    ...
    def build_train_op(self, params, optim, lr, max_grad_norm=0.5):
        pg_fisher_loss = -tf.reduce_mean(self.fn_neg_log_prob(self.A))
        sample_net = self.model_vf + tf.random_normal(tf.shape(self.model_vf))
        vf_fisher_loss = - 1.0 * tf.reduce_mean(tf.pow(self.model_vf - tf.stop_gradient(sample_net), 2))
        ...
        with tf.device('/gpu:0'):
            ...
            self.train_op, _ = optim.apply_gradients(grads)
        self.LR = lr
